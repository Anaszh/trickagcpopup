...
function AGCZoneContent()
{
	...
	if(@simplexml_load_string($ama))
	{
		...
		$result .= '<div class="agczone-thumbnail"><a href="http://www.amazon.'.$reg.'/gp/product/'.$xml[0]->ASIN.'/'.$tag.'" rel="nofollow" target="_blank"><img class="agczone-left" src="'.$xml[0]->MediumImage->URL.'" data-src="'.$xml[0]->MediumImage->URL.'" width="'.$xml[0]->MediumImage->Width.'" height="'.$xml[0]->MediumImage->Height.'" alt="'.$xml[0]->ItemAttributes->Title.'" /></a></div>' . "\n";
		$result .= '<div class="agczone-details">' . "\n";
		$result .= '<table class="agczone-table">';
		$result .= '<tbody>';
		$result .= '<tr style="background:#eee;width:100%;">' . "\n";
		$result .= '<td class="agczone-td">List Price</td>' . "\n";
		$result .= '<td>:</td>' . "\n";
		...
		$result .= '<p style="text-align:center"><a href="http://www.amazon.'.$reg.'/gp/product/'.$xml[0]->ASIN.'/'.$tag.'" rel="nofollow" target="_blank"><img src="'.plugins_url('/buy-now.png', __FILE__).'" data-src="'.plugins_url('/images/buy-now.png', __FILE__).'" /></a></p>' . "\n";
...
{
	$sheeturl = plugins_url('/agczoneonesheet.css', __FILE__);
	$moourl = plugins_url('/mootools.js', __FILE__);
	$lazyurl = plugins_url('/Lazy.js', __FILE__);
